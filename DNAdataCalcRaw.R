...
voter_prop_mat_selidx <- which(tolower(all_voters) %in% tolower(aird_list))
...
voter_prop_mat_selidx <- which(apply(voter_prop_mat_sel,1,function(x) sum(!is.na(x)))>50)
sel_voters <- sel_voters[voter_prop_mat_selidx]
voter_prop_mat_sel <- voter_prop_mat_sel[voter_prop_mat_selidx,]
...
voter_score_df <- voter_score_df[voter_score_df$Score>.75,]
...
voter_score_df$voterAName <- ifelse((A_match=="")|is.na(A_match),voter_score_df$voterA,A_match)
B_match <- del_data$Name[match(voter_score_df$voterB,del_data$Address)]
voter_score_df$voterBName <- ifelse((B_match=="")|is.na(B_match),voter_score_df$voterB,B_match)
